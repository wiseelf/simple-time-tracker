...
// Keyboard-editable time field that works with minute offsets (0…1440).
// Displays as HH:MM; supports 24:00 for end-of-day schedules.

struct MinutePickerField: NSViewRepresentable {
    @Binding var minutes: Int

    func makeNSView(context: Context) -> MinutePickerNSView {
        let v = MinutePickerNSView()
        v.onMinutesChange = { [self] m in minutes = m }
        v.setMinutes(minutes)
        return v
    }

    func updateNSView(_ nsView: MinutePickerNSView, context: Context) {
        nsView.setMinutes(minutes)
    }

    static func dismantleNSView(_ nsView: MinutePickerNSView, coordinator: ()) {
        nsView.cleanup()
    }
}
...
final class MinutePickerNSView: NSView {
    ...
    func setMinutes(_ m: Int) {
        let clamped = max(0, min(1440, m))
        let h = clamped / 60
        let min = clamped % 60
        guard h != hour || min != minute else { return }
        hour = h; minute = min; needsDisplay = true
    }
    ...
    private func adjust(by delta: Int) {
        if segment == .hour {
            hour = max(0, min(24, hour + delta))
            if hour == 24 { minute = 0 }
        } else {
            guard hour < 24 else { return }
            minute = max(0, min(59, minute + delta))
        }
    ...
    private func handleDigit(_ digit: String) {
        ...
        if segment == .hour {
            if let val = Int(pending) {
                let clamped = min(val, 24)
                if pending.count == 2 || dv > 2 {
                    hour = clamped
                    if hour == 24 { minute = 0 }
                    pendingInput = ""
                    switchTo(.minute)
