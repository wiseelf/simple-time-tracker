...
- **Statistics** — bar chart per day (week view) or per calendar week (month view); period total and daily average count regular (non on-call) time only, with on-call active time shown as an appended orange segment; optional income footer
- **On-call billing** — define rotation blocks with per-day-of-week schedules; global non-billable window; passive on-call derived automatically; click "On-call" while running to split into an active on-call segment — active time counts as on-call whether or not it falls inside a scheduled rotation window, so ad hoc escalations are tracked too; configurable passive/active rate multipliers; optional income tracking with rate history
...
- **Shared panel builder** — `makePanel(contentView:width:height:topLeft:)` builds the `KeyablePanel: NSPanel` shell (`.borderless` + `.nonactivatingPanel` style masks, frosted-glass `NSVisualEffectView` with `.popover` material and 12 pt corner radius, `sharingType` from `hideFromScreenCapture`) used by both the main panel and the session detail panel, so chrome and screen-capture exclusion can't drift between them. Callers own sizing, positioning, key/activation behavior, and their own event handling.
- **Main panel** — built via the shared panel builder. Height is capped to the available screen space below the menu bar.
- **Session detail panel** — a second panel built via the shared panel builder, opening to the right of the main panel when tapping a day in Stats. Uses `orderFront` (not `makeKeyAndOrderFront`) to avoid stealing key focus. Backed by `SessionDetailState` so the date updates in place without recreating the panel. Closes automatically when the main panel closes.
